...
Meteor.publish('territories.by_id', function (id) {
    return Territories.find({ _id: id }, {limit: 1000, sort: {priority: 1}})
})

Meteor.publish('territories.by_ids', function (ids) {
    return Territories.find({ _id: {$in: ids} }, {limit: 1000, sort: {priority: 1}})
})

Meteor.publish('territories.active', function() {
    return Territories.find({active: true}, {limit: 1000, sort: {priority: 1}})
})

Meteor.publish('territories.by_shorten_url', function(shorten_url) {
    return Territories.find({shorten_url}, {limit: 1000, sort: {priority: 1}})
})
